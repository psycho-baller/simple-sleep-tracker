...
struct SleepSessionEditorView: View {
    ...
    init(session: BlockedProfileSession?, profile: BlockedProfiles) {
        ...
        if let session = session {
            ...
            // Assume "last night" means starting yesterday evening?
            // Or just default to optimal sleep time?
            // Let's look up optimal sleep time from settings if possible, but for now just 10pm-6am
            let yesterday = calendar.date(byAdding: .day, value: -1, to: now)!
            let start = calendar.date(bySettingHour: 22, minute: 0, second: 0, of: yesterday)!
            let end = calendar.date(bySettingHour: 7, minute: 0, second: 0, of: now)!
